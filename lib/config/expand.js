...
module.exports = {
    ...
    expandTransformers: function expandTransformers(transformers) {
        ...
        else if (R.is(Object, transformers)) return expandObjectTransformers(transformers)
    }
}
...
function expandObjectTransformers(transformers) {
    return R.keys(transformers).reduce(function(entries, key) {
        var value = transformers[key]
        var expanded = {
            transformer: expandTransformer(value, key)
        }
        return entries.concat(expanded)
    }, [], transformers)
}
